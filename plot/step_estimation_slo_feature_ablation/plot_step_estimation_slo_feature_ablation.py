...
OUTPUT_STEM = "step_estimation_slo_feature_ablation_1x2"
...
def print_oracle_gaps(rows: list[dict[str, object]]) -> None:
    grouped_by_multiple: dict[float, dict[str, dict[str, object]]] = {}
    for row in rows:
        slo_multiple = float(row["slo_multiple"])
        grouped_by_multiple.setdefault(slo_multiple, {})[str(row["baseline"])] = row

    print("SLO-attainment gap vs oracle by SLO multiple:")
    for slo_multiple in sorted(grouped_by_multiple, reverse=True):
        rows_by_baseline = grouped_by_multiple[slo_multiple]
        oracle_row = rows_by_baseline.get("oracle")
        if oracle_row is None:
            continue

        oracle_value = float(oracle_row["slo_attainment"])
        gap_parts = []
        for baseline in ["one_shot", "no_conf", "all_features"]:
            row = rows_by_baseline.get(baseline)
            if row is None:
                continue
            gap_pp = (float(row["slo_attainment"]) - oracle_value) * 100.0
            gap_parts.append(f"{LEGEND_LABELS[baseline]}: {gap_pp:+.1f} pp")

        print(
            f"  {format_multiple_tick(slo_multiple)} | "
            f"Oracle: {oracle_value * 100:.1f}% | "
            + "; ".join(gap_parts)
        )
...
def plot_feature_ablation_panel(ax) -> None:
    ...
    bars = ax.barh(positions, values, color=colors, height=0.85, edgecolor="black")
...
def main(summary_path: Path, out_dir: Path) -> None:
    ...
    axes[0].set_ylabel("SLO Attain. (%)", labelpad=1)
    axes[0].set_xlabel("SLO Multiple", labelpad=1)
    axes[0].tick_params(axis="both", direction="in", pad=2.0)
    axes[0].yaxis.set_major_formatter(PERCENT_INTEGER_FORMATTER)

    plot_feature_ablation_panel(axes[1])

    legend_handles = [
        Patch(facecolor=BASE_COLORS[key], edgecolor="black", label=LEGEND_LABELS[key])
    ...
    fig.subplots_adjust(left=0.1, right=0.995, bottom=0.24, top=0.755, wspace=0.48)

    # Shrink the right plotting box itself and shift it right so the long
    # category labels have more room without changing the overall canvas size.
    right_pos = axes[1].get_position()
    label_room = 0.06
    axes[1].set_position(
        [
            right_pos.x0 + label_room,
            right_pos.y0,
    ...
    fig.savefig(out_dir / f"{OUTPUT_STEM}.png", dpi=300, bbox_inches="tight", pad_inches=0.02)
    fig.savefig(out_dir / f"{OUTPUT_STEM}.pdf", bbox_inches="tight", pad_inches=0.02)
    plt.close(fig)
