...
def classify_response(response):
    ...
    if isinstance(error_payload, dict):
        ...
        if message and "drop" in message.lower():
            return "dropped", None
        return "error", None
...
def calculate_throughput(queries,
    ...
    for prompt, response in queries:
        status, assistant_text = classify_response(response)
        ...
        if status == "success" and assistant_text is not None:
            successful_prompts.append(prompt)
            successful_responses.append(assistant_text)
    ...
    try:
        prompt_lens = get_tok_id_lens(tokenizer, successful_prompts)
        response_lens = get_tok_id_lens(tokenizer, successful_responses)
    except Exception:
    ...
    qps = len(successful_responses) / dur_s
    msg1 = f'dur_s {dur_s:.04f} tokens_per_s {throughput_tok_s:.02f} qps {qps:.04f}\n'
    dropped_responses = sum(record["status"] == "dropped" for record in request_records)
    errored_responses = sum(record["status"] == "error" for record in request_records)
    msg2 = (
        f'successful_responses {len(successful_responses)} '
        f'dropped_responses {dropped_responses} '
        f'errored_responses {errored_responses} '
        f'prompt_token_count {prompt_token_count} response_token_count {response_token_count}\n'
    )
    ...
    if fail_on_response_failure:
        assert len(successful_responses) == len(queries), \
            f"{fail_on_response_failure=}, expected number of successful respones to equal number of queries, got {len(successful_responses)} vs {len(queries)}"

    return throughput_tok_s, successful_prompts, successful_responses, request_records
...
async def benchmark(
    ...
    throughput, prompts, responses, request_records = calculate_throughput(queries,
                                      dur_s,
                                      tokenizer,
                                      median_token_latency,
                                      median_e2e_latency,
...
def main():
    ...
    arrival_times, \
    completion_times, \
    request_records = asyncio.run(benchmark(
        tokenizer,
        prompts,
        arrival_times,
        args.verbose,
    ...
    instances = {
        str(i): {
            ...
            "arrival_time": (
                datetime.datetime.fromtimestamp(arrival_times[i]).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
                if arrival_times[i] is not None else None
            ),
            "completion_time": (
                datetime.datetime.fromtimestamp(completion_times[i]).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
                if completion_times[i] is not None else None
            ),
            "prompt": request_records[i]["prompt"],
            "response": request_records[i]["response"],
            "status": request_records[i]["status"],
            "raw_response": request_records[i]["raw_response"],
        }
        for i in range(len(request_records))
    }
