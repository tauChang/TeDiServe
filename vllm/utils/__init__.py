...
_OPEN_PORT_LOCK = threading.Lock()
_RESERVED_OPEN_PORTS: set[int] = set()
...
def _get_open_port() -> int:
    with _OPEN_PORT_LOCK:
        ...
        if port is not None:
            while True:
                if port in _RESERVED_OPEN_PORTS:
                    logger.info("Port %d is reserved, trying port %d", port, port + 1)
                    port += 1
                    continue
