        ...
        with self.schedule_profiler.section("total"):
            ...
            with self.schedule_profiler.section("stage 6"):
                ...
                for executor_id in idle_executors + idle_not_accepting_executors:
                    ...
                    # confs = [0.9, 0.9, 0.9]
                    high_conf_chance = 1.01
                    for req_id in num_scheduled_tokens[executor_id].keys():

                        # unmask_progress = self.requests[req_id].unmask_progress
                        # if unmask_progress < 0.33:
                        #     conf = confs[0]
                        # elif unmask_progress < 0.67:
                        #     conf = confs[1]
                        # else:
                        #     conf = confs[2]
                        # confidence_thresholds[req_id] = conf
                        # 10% chance 0.9, 90% chance 0.5
                        confidence_thresholds[req_id] = 0.6 if random.random() < (1 - high_conf_chance) else 0.8
