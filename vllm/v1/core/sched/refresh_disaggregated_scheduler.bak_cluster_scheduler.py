...
class RequestState:
    ...
    def set_refresh_executor(self, executor_id: int) -> None:
        logger.debug(f"Request {self.request.request_id} setting refresh executor to {executor_id}")
        self.refresh_executor_id = executor_id
        logger.debug(f"after setting refresh, request state: {self}")
    ...
    def remove_refresh_executor(self) -> None:
        logger.debug(f"Request {self.request.request_id} clearing refresh executor {self.refresh_executor_id}")
        self.refresh_executor_id = None
        logger.debug(f"after clearing refresh, request state: {self}")
...
class RefreshDisaggregatedScheduler(SchedulerInterface):
    ...
    def __init__(
        ...
        assert self.cache_prefix, "RefreshDisaggregatedScheduler is only used for approximate caching"
