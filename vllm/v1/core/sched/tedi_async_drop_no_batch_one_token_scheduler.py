...
class TeDiLightScheduler(SchedulerInterface):
    ...
    def __init__(
        ...
        self.enable_dropping = self.scheduler_config.enable_dropping
        ...
        self._dropped_deferred_requests: deque[Request] = deque()
    ...
    def _admit_deferred_requests(self) -> None:
        expired_request_ids = {
            request.request_id for request in self.deferred_requests
            if request.slo_time_remaining <= 0
        }
        self._drop_deferred_requests(expired_request_ids)
    ...
    def _drop_deferred_requests(self,
                                request_ids: set[str],
                                *,
                                force: bool = False) -> None:
        if not request_ids:
            return
        if not self.enable_dropping and not force:
            return
        kept_requests: deque[Request] = deque()
        dropped_any = False
        for request in self.deferred_requests:
            if request.request_id in request_ids:
                logger.debug("Dropping deferred request %s.",
                             request.request_id)
                self._drop_deferred_request(request)
                dropped_any = True
                continue
    ...
    def _drop_deferred_request(self, request: Request) -> None:
        req_id = request.request_id

        self._dropped_deferred_requests.append(request)

        if req_id in self.deferred_request_states:
            self.deferred_request_states.pop(req_id, None)

        if req_id in self.requests:
            self.requests.pop(req_id, None)

        with self._step_estimate_condition:
            self._step_estimate_pending_req_ids.discard(req_id)
            if not self._step_estimate_pending_req_ids:
                self._step_estimate_condition.notify_all()

    def _get_deferred_request_ids(self, request_ids: set[str]) -> set[str]:
        deferred_request_ids: set[str] = set()

        for req_id in request_ids:
            if req_id in self.deferred_request_states and req_id not in self.request_states:
                deferred_request_ids.add(req_id)

        if self.deferred_requests:
            deferred_queue_ids = {
                request.request_id for request in self.deferred_requests
            }
            deferred_request_ids.update(request_ids.intersection(deferred_queue_ids))

        return deferred_request_ids

    def _make_drop_flush_scheduler_output(self) -> dict[int, SchedulerOutput]:
        if not self.executor_states:
            return {}

        executor_id = next(iter(self.executor_states))
        ...
        return {executor_id: scheduler_output}
    ...
    def _step_estimate_worker_loop(self) -> None:
        while True:
            ...
            self._process_step_estimate_updates()
    ...
    async def schedule(self) -> SchedulerOutput:
        ...
        def find_best_executor(candidate_executors: list[int], req_id: str, 
            ...
            if cur_executor_id is not None and len(candidate_executors) > 0:
                # one shot
                candidate_executors = [candidate_executors[0]]
            ...
            for ex_id in candidate_executors:
                ...
                if self.executor_states[ex_id].tp_degree > 1:
                    projected_req_ids = self.executor_states[ex_id].get_projected_req_ids()
                    max_projected_reqs = 1
                    if len(projected_req_ids) >= max_projected_reqs:
                        logger.debug(
                            "Can't consider executor %s because TP=%s only allows %s projected requests. Existing projected req_ids=%s",
                            ex_id,
        ...
        if not scheduler_outputs and self._dropped_deferred_requests:
            scheduler_outputs = self._make_drop_flush_scheduler_output()
        ...
        # log number of scheduled tokens for each executor
        # for executor_id, output in scheduler_outputs.items():
        #     logger.info(
        #         f"Scheduled executor {executor_id} with total_num_scheduled_tokens: {output.total_num_scheduled_tokens}, num_scheduled_tokens: {output.num_scheduled_tokens}")
    ...
    async def update_from_output(
        ...
        with self.update_profiler.section("total"):
            while self._dropped_deferred_requests:
                dropped_request = self._dropped_deferred_requests.popleft()
                outputs[dropped_request.client_index].append(
                    EngineCoreOutput(
                        request_id=dropped_request.request_id,
                        new_token_ids=[],
                        finish_reason=FinishReason.ABORT,
                        stop_reason=None,
                        events=dropped_request.take_events(),
                        kv_transfer_params=None,
                        num_cached_tokens=dropped_request.num_cached_tokens,
                        num_denoise_ran=dropped_request.num_denoise_ran,
                    ))
    ...
    def finish_requests(
        ...
        deferred_request_ids = self._get_deferred_request_ids(request_ids)
        if deferred_request_ids:
            self._drop_deferred_requests(deferred_request_ids, force=True)
            request_ids -= deferred_request_ids

        self._drop_deferred_requests(set(request_ids), force=True)
    ...
    def has_finished_requests(self) -> bool:
        ...
        logger.debug("in has_finished_requests, free_req_ids: %s, dropped_deferred_requests: %s",
                     self.free_req_ids, self._dropped_deferred_requests)
        return bool(self._dropped_deferred_requests) or any(
            len(free) > 0 for free in self.free_req_ids.values())
