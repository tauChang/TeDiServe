...
class Scheduler(SchedulerInterface):
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        req_index = 0
        print(f"start scheduling running requests, ")
        while req_index < len(self.running) and token_budget > 0:
            print(f"req_index: {req_index}, token_budget: {token_budget}, "
                  f"running len: {len(self.running)}")
            ...
            print(f"request.num_tokens_with_spec: {request.num_tokens_with_spec}, request.num_computed_tokens: {request.num_computed_tokens}, num_new_tokens: {num_new_tokens}, ")
        ...
        print(f"start scheduling waiting requests, ")
        if not preempted_reqs:
            print(f"is preempted_reqs empty, ")
            print(f"len(self.waiting): {len(self.waiting)}, token_budget: {token_budget}, ")
                ...
                # Number of tokens to be scheduled.
                    ...
                    if (0 < self.scheduler_config.long_prefill_token_threshold
                        ...
                        num_new_tokens = (
                            self.scheduler_config.long_prefill_token_threshold)
                    num_new_tokens = min(num_new_tokens, token_budget)
                    print(f"request.num_tokens: {request.num_tokens}, num_computed_tokens: {num_computed_tokens}, num_new_tokens: {num_new_tokens}, ")
    ...
    def get_num_unfinished_requests(self) -> int:
        logger.debug(f"get_num_unfinished_requests: "
                        f"len(self.running)={len(self.running)}, "
                        f"len(self.waiting)={len(self.waiting)}, "
                        f"len(self.finished_req_ids)={len(self.finished_req_ids)}")
        return len(self.waiting) + len(self.running)

    def has_finished_requests(self) -> bool:
        logger.debug(f"has_finished_requests: "
                     f"finished_req_ids={self.finished_req_ids}, "
                     f"len(self.finished_req_ids)={len(self.finished_req_ids)}")
