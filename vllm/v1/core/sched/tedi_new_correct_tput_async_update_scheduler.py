...
class TeDiLightScheduler(SchedulerInterface):
    ...
    def __init__(
        ...
        self.max_confidence_threshold_logger = MaxConfidenceThresholdLogger(
            "max_confidence_threshold_logs", self)
        ...
        self.confidence_threshold_tput_demand_change_ratio = (
            self.scheduler_config.confidence_threshold_tput_demand_change_ratio)
        self.sync_step_prediction = self.scheduler_config.sync_step_prediction
        self.current_system_tput_demand: float = 0.0
        self._request_tput_demand_contributions: dict[str, float] = {}
        self._last_confidence_threshold_recompute_tput_demand: float = 0.0
        self._last_confidence_threshold_recompute_throughput_supply: float = 0.0
        self._force_update_request_max_confidence_thresholds = False
    ...
    def _admit_request(self, request: Request) -> None:
        ...
        initial_pred_num_steps_left = self.step_estimator.get_initial_default_predictions(
            self.request_states[request.request_id].last_stats)
        self.request_states[request.request_id].pred_num_steps_left = initial_pred_num_steps_left

        request_tput_demand = self._estimate_new_request_tput_demand_contribution(request.request_id)
        self._request_tput_demand_contributions[request.request_id] = request_tput_demand
        self.current_system_tput_demand += request_tput_demand

        logger.debug(
            "Added request %s to aggregate demand contribution=%s; current aggregate demand=%s",
            request.request_id,
            request_tput_demand,
            self.current_system_tput_demand,
        )
        if self.log_stats:
    ...
    def _get_request_confidence_for_tput_demand(self, req_id: str) -> float:
        req_state = self.request_states.get(req_id)
        if req_state is None:
            return 0.0
        if req_state.confidence_threshold is not None:
            return req_state.confidence_threshold
        return self.candidate_confidence_thresholds[self.request_states[req_id].max_confidence_threshold_idx]

    def _estimate_new_request_tput_demand_contribution(self, req_id: str) -> float:
        req = self.requests.get(req_id)
        req_state = self.request_states.get(req_id)
        if req is None or req_state is None:
            return 0.0

        current_tput_demand = self.current_system_tput_demand
        req_tput_demand = None
        confidence_used = None
        for idx, confidence in enumerate(self.candidate_confidence_thresholds):
            if idx > req_state.max_confidence_threshold_idx:
                break
            req_tput_demand = self._get_request_tput_demand(req_id, confidence)
            confidence_used = confidence
            if current_tput_demand + req_tput_demand <= self.throughput_supply:
                break
        
        logger.debug(
            "Estimated new tput demand contribution for req %s: %s at confidence %s with current_system_tput_demand %s and throughput_supply %s",
            req_id,
            req_tput_demand,
            confidence_used,
            current_tput_demand,
            self.throughput_supply,
        )
        assert req_tput_demand is not None
        return req_tput_demand
        
    def _should_update_request_max_confidence_thresholds(self) -> bool:
        if self._force_update_request_max_confidence_thresholds:
            logger.debug(
                "Confidence-threshold recompute check: forced by executor topology change.")
            return True
        if not self.request_states:
            return False
        if not any(
            req_state.confidence_threshold_dirty
            for req_state in self.request_states.values()):
            return False

        current_tput_demand = self.current_system_tput_demand
        baseline_tput_demand = self._last_confidence_threshold_recompute_tput_demand
        if baseline_tput_demand <= 0:
            logger.debug(
                "Confidence-threshold recompute check: no baseline demand yet; current aggregate demand=%s.",
                current_tput_demand,
            )
            return current_tput_demand > 0

        demand_change_ratio = abs(current_tput_demand - baseline_tput_demand) / baseline_tput_demand
        logger.debug(
            "Confidence-threshold recompute check: demand current=%s baseline=%s ratio=%.4f threshold=%.4f",
            current_tput_demand,
            baseline_tput_demand,
            demand_change_ratio,
            self.confidence_threshold_tput_demand_change_ratio,
        )
        if demand_change_ratio >= self.confidence_threshold_tput_demand_change_ratio:
            return True
    ...
    def is_possible_to_meet_slo(self, req_id: str):
        pred_num_steps_left = self.request_states[req_id].pred_num_steps_left[self.candidate_confidence_thresholds[-1]]
    ...
    def update_request_max_confidence_thresholds(self) -> None:
        ...
        self.current_system_tput_demand = 0.0
        self._request_tput_demand_contributions = {}
        for i, req_id in enumerate(reqs_by_progress):
            ...
            if not self.is_possible_to_meet_slo(req_id):
                ...
                for idx, conf in enumerate(self.candidate_confidence_thresholds):
                    ...
                    if self.current_system_tput_demand + tput_demand + cum_min_tput_suffix[i] <= total_throughput_budget:
                        break
            ...
            self._request_tput_demand_contributions[req_id] = tput_demand
            self.current_system_tput_demand += tput_demand
            # else:
        ...
        # logging
        for req_id in self._request_tput_demand_contributions:
            logger.debug(
                "Request %s max confidence threshold set to %s with tput demand contribution %s",
                req_id,
                self.candidate_confidence_thresholds[
                    self.request_states[req_id].max_confidence_threshold_idx],
                self._request_tput_demand_contributions[req_id],
            )

        self._last_confidence_threshold_recompute_tput_demand = self.current_system_tput_demand
        self._last_confidence_threshold_recompute_throughput_supply = self.throughput_supply
        self._force_update_request_max_confidence_thresholds = False

        request_max_confidence_thresholds = {
            req_id: self.candidate_confidence_thresholds[
                self.request_states[req_id].max_confidence_threshold_idx]
            for req_id in reqs_by_progress
        }
        request_max_confidence_threshold_idx = {
            req_id: self.request_states[req_id].max_confidence_threshold_idx
            for req_id in reqs_by_progress
        }
        self.max_confidence_threshold_logger.log(
            MaxConfidenceThresholdSnapshot(
                ...
                request_max_confidence_thresholds=
                    request_max_confidence_thresholds,
                request_tput_demand=self._request_tput_demand_contributions.copy(),
    ...
    async def schedule(self) -> SchedulerOutput:
        ...
        def find_best_executor(candidate_executors: list[int], req_id: str, 
            ...
            if cur_executor_id is not None:
                cur_batch_size = self.executor_states[
                    cur_executor_id].get_projected_batch_size()
                profile_res = self.get_profile_latency(
                    self.executor_states[cur_executor_id].tp_degree,
                    cur_batch_size)
                if profile_res is not None:
                    cur_step_latency = profile_res[1]
                else:
            ...
            candidate_executors.sort(
                key=lambda ex_id: self.executor_states[ex_id].
                get_projected_batch_size(),
            )
            ...
            for ex_id in candidate_executors:
                ...
                projected_batch_size = self.executor_states[
                    ex_id].get_projected_batch_size()
                ...
                if profile_res is None:
                    logger.debug(
                        "Can't consider executor %s because batch size %s is not in profile. Skipping.",
                        ex_id, projected_batch_size + num_tokens)
                    continue

                migrated_step_latency = profile_res[1]
                logger.debug("migrated_step_latency: %s",
        ...
        with self.schedule_profiler.section("total"):
            ...
            with self.schedule_profiler.section("update requests"):
                if self._should_update_request_max_confidence_thresholds():
                    logger.debug(
                        "Triggering max-confidence recompute: aggregate demand=%s baseline=%s throughput=%s",
                        self.current_system_tput_demand,
                        self._last_confidence_threshold_recompute_tput_demand,
                        self.throughput_supply,
                    )
                    self.update_request_max_confidence_thresholds() # needs step estimates
            ...
            with self.schedule_profiler.section("stage 6"):
                ...
                for executor_id in idle_executors + idle_not_accepting_executors:
                    ...
                    for req_id in num_scheduled_tokens[executor_id]:
                        ...
                        logger.debug(
                            "Set confidence threshold for req %s on executor %s to %s; aggregate demand=%s",
                            req_id,
                            executor_id,
                            best_conf,
                            self.current_system_tput_demand,
    ...
    def finish_requests(
        ...
        for request in valid_requests:
            ...
            logger.debug(
                "Finished request %s processed; current aggregate demand=%s",
                request.request_id,
                self.current_system_tput_demand,
            )
    ...
    def _free_request(self, request: Request, 
        ...
        if finished:
            old_tput_demand = self._request_tput_demand_contributions.pop(request.request_id, 0.0)
            self.current_system_tput_demand -= old_tput_demand
            logger.debug(
                "Removed finished request %s contribution=%s aggregate demand=%s",
                request.request_id,
                old_tput_demand,
                self.current_system_tput_demand,
            )
    ...
    def add_executor(self, executor_id: int) -> None:
        ...
        self._force_update_request_max_confidence_thresholds = True
