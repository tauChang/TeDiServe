...
class RequestState:
    def __init__(self, request: Request) -> None:
        ...
        self.last_confidence_threshold_update = -1
...
class TeDiLightScheduler(SchedulerInterface):
    ...
    def __init__(
        ...
        self.confidence_threshold_update_interval = 0.5
    ...
    def update_throughput_supply(self) -> None:
        ...
        self.throughput_supply = sum
        self.max_throughput_demand_per_req = max_tput_per_req
        logger.debug(f"Updated throughput supply: {self.throughput_supply}")
        logger.debug(f"Updated max throughput demand per req: {self.max_throughput_demand_per_req}")
    
        logger.debug(f"update_throughput_supply: {time.time() - start_time} seconds")

    def update_step_estimates_batch(self, req_ids: list[str]) -> None:
        all_stats = []
        index_map = []   # (req_id, conf)
        ...
        for req_id in req_ids:
            ...
            for conf in self.candidate_confidence_thresholds:
                stats = StepStats(
                    ...
                    output_min_confidence=req.last_stats.output_min_confidence,
                    output_q25_confidence=req.last_stats.output_q25_confidence,
                    output_median_confidence=req.last_stats.output_median_confidence,
                    output_q75_confidence=req.last_stats.output_q75_confidence,
                    output_avg_confidence=req.last_stats.output_avg_confidence,
    ...
    def get_estimated_time_left(self, 
        ...
        # make pessemistic if early
        # if self.requests[req_id].unmask_progress < 0.3:
        #     num_cache_steps_left *= 1.3
    ...
    def update_request_max_confidence_thresholds(self) -> None:
        def get_tput_demand(r_id: str, confidence: float) -> float:
            ...
            tput_demand = (num_recompute_steps_left * self.requests[r_id].num_tokens + \
                num_cache_steps_left * self.requests[r_id].denoise_block_size)
            # tput_demand = (num_recompute_steps_left * self.requests[r_id].num_tokens + \
        ...
        total_throughput_budget = self.throughput_supply / max(self.get_rps(), 0.1) * len(self.requests) * 0.8
        # logger.debug(
        ...
        for i, req_id in enumerate(reqs_by_progress):
            ...
            if not self.is_possible_to_meet_slo(req_id):
                ...
                cum_tput_demand += get_tput_demand(req_id, self.candidate_confidence_thresholds[-1])
                continue
            ...
            for idx, conf in enumerate(self.candidate_confidence_thresholds):
                ...
                if cum_tput_demand + tput_demand + cum_min_tput_suffix[i] <= total_throughput_budget:
    ...
    async def schedule(self) -> SchedulerOutput:
        ...
        def slo_met(req_id: str,
            ...
            if est_time_left > slo_time_remaining:
                ...
                logger.debug(f"slo_met took {time.time() - start_time} seconds")
                return False
            logger.debug(f"slo_met took {time.time() - start_time} seconds")
            return True
        ...
        with self.schedule_profiler.section("total"):
            ...
            with self.schedule_profiler.section("update requests"):
                if self.request_added_or_removed and \
                     time.time() - self.last_update_request > self.update_request_interval:
            ...
            with self.schedule_profiler.section("stage 6"):
                ...
                for executor_id in idle_executors + idle_not_accepting_executors:
                    ...
                    for req_id in num_scheduled_tokens[executor_id].keys():
                        if self.request_states[req_id].confidence_threshold is None or \
                            time.time() - self.request_states[req_id].last_confidence_threshold_update > self.confidence_threshold_update_interval:
                            ...
                            if not self.is_possible_to_meet_slo(req_id):
                                ...
                                for conf in self.candidate_confidence_thresholds[max_conf_idx:]:
                                    ...
                                    if slo_met(req_id,
                                                ...
                                                conf):
                                        break
                            self.request_states[req_id].last_confidence_threshold_update = time.time()
                        else:
                            best_conf = self.request_states[req_id].confidence_threshold
                            
                        assert best_conf is not None
                        confidence_thresholds[req_id] = best_conf
                        self.request_states[req_id].confidence_threshold = best_conf
                        logger.debug(f"for req {req_id} on executor {executor_id}, setting confidence threshold to {best_conf}")
