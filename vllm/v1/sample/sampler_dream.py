...
class Sampler(nn.Module):
    ...
    def unmask(
        ...
        # CRITICAL FIX: Shift logits by 1 position
        # In autoregressive LLMs, logits at position i predict token at position i+1
        # We need logits[i-1] to predict what token should be at position i
        # So we shift: new_logits[i] = old_logits[i-1], and new_logits[0] = old_logits[0]
        shifted_logits = torch.cat([logits_with_noise[:1, :], logits_with_noise[:-1, :]], dim=0)
        
        x_0 = torch.argmax(shifted_logits, dim=-1)
