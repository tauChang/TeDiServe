...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def _prepare_inputs(
        ...
        for kv_cache_group_id, kv_cache_group_spec in enumerate(
            ...
            # [tau_chang] ignore cascade attention for now.
    ...
    def execute_model(
        ...
        try:
            ...
            self._sync_device()
            logger.debug(f"Preparation for model forward took "
                        f"{time.time() - crap_start_time} seconds")
            ...
            with set_forward_context(
                ...
                self._sync_device()
                setup_kv_start_time = time.time()
                self.maybe_setup_kv_connector(scheduler_output)
                self._sync_device()
                logger.debug(f"maybe_setup_kv_connector took "
                                f"{time.time() - setup_kv_start_time} seconds")
                ...
                logger.debug(f"Model forward took {time.time() - model_start_time} "
                            f"seconds")
                with open(f"executor_profiles/1_pd.txt", "a") as f:
                    # write input_ids size and time
                    f.write(f"Model forward: {time.time() - model_start_time}\n")

                self._sync_device()
                finish_kv_start_time = time.time()
                self.maybe_wait_for_kv_save()
                finished_sending, finished_recving = (
                    self.get_finished_kv_transfers(scheduler_output))
                self._sync_device()
                logger.debug(f"Finishing KV transfer took "
                            f"{time.time() - finish_kv_start_time} seconds")
            ...
            if spec_decode_metadata is None:
                try:
                    logger.debug(f"len(input_ids): {len(input_ids)}")
    ...
    def maybe_setup_kv_connector(scheduler_output: "SchedulerOutput"):
        ...
        if has_kv_transfer_group():
            ...
            if len(scheduler_output.kv_connector_metadata.reqs_to_recv) > 0:
                with open(f"executor_profiles/1_pd.txt", "a") as f:
                    # write input_ids size and time
                    f.write(f"Num of reqs to recv: {len(scheduler_output.kv_connector_metadata.reqs_to_recv)}. Overhead: {overhead}\n")
    ...
    def kv_connector_no_forward(
        ...
        logger.debug(f"kv_connector_no_forward took {time.time() - start_time} seconds")
        return output
