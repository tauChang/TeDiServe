#!/bin/bash
#SBATCH --job-name=dllm-sharegpt-multi
...
CONFIG_FILE=${CONFIG_FILE:-./sbatch_config/gsm8k_ablation_8workers.json}
...
export RAY_PORT RAY_NUM_CPUS

ln -sfn "$SBATCH_DIR" "$SCRIPT_DIR/current_experiment"
cp "$SCRIPT_DIR/run_sharegpt_multi_sbatch.sh" "$SBATCH_DIR/"
...
stop_ray_cluster() {
    ...
    if [ "$num_nodes" -gt 1 ]; then
        srun --nodes="$num_nodes" --ntasks="$num_nodes" --ntasks-per-node=1 \
            --kill-on-bad-exit=0 bash -lc 'ray stop --force >/dev/null 2>&1 || true' \
            >/dev/null 2>&1 || true
    fi
}
...
# Clean up stale Ray state from previous jobs on reused nodes.
ray stop --force >/dev/null 2>&1 || true

if [[ "$node_name" == "$RAY_HEAD_NODE" ]]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] starting Ray head on ${node_name}"
    ray start --head --port="$RAY_PORT" --num-cpus="$RAY_NUM_CPUS" --block
else
    sleep 15
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] starting Ray worker on ${node_name}, connecting to ${RAY_HEAD_ADDRESS}:${RAY_PORT}"
    ray start --address="${RAY_HEAD_ADDRESS}:${RAY_PORT}" --num-cpus="$RAY_NUM_CPUS" --block
fi
EOF
...
run_one_experiment() {
    ...
    export VLLM_PORT=${VLLM_PORT:-8083}
    ...
    export ENABLE_DROPPING=${ENABLE_DROPPING:-true}
    export STEP_ESTIMATOR_REFRESH_UNMASKED_TOKEN_DELTA=${STEP_ESTIMATOR_REFRESH_UNMASKED_TOKEN_DELTA:-8}
    ...
    export CONFIDENCE_THRESHOLD_TPUT_DEMAND_CHANGE_RATIO=${CONFIDENCE_THRESHOLD_TPUT_DEMAND_CHANGE_RATIO:-0.1}
    export RECONFIG_INTERVAL=${RECONFIG_INTERVAL:--1}
    export SLO=${SLO:-10}
    export MAX_NUM_UNFINISHED_REQUESTS=${MAX_NUM_UNFINISHED_REQUESTS:-50}
    ...
    export REQUEST_PLOTS_DIR="$run_dir/request_plots"

    # Determine step estimator paths based on cache settings
    if [ "$CACHE_PREFIX" = true ] && [ "$CACHE_SUFFIX" = true ]; then
        STEP_ESTIMATOR_MODEL_PATH=${STEP_ESTIMATOR_MODEL_PATH:-./analysis/denoise_step_prediction/models/lgb/sharegpt_0423_all_features/model.bin}
        STEP_ESTIMATOR_FEATURES_PATH=${STEP_ESTIMATOR_FEATURES_PATH:-./analysis/denoise_step_prediction/models/lgb/sharegpt_0423_all_features/features.txt}
    elif [ "$CACHE_PREFIX" = true ] && [ "$CACHE_SUFFIX" = false ]; then
        STEP_ESTIMATOR_MODEL_PATH=${STEP_ESTIMATOR_MODEL_PATH:-./analysis/denoise_step_prediction/models/lgb/prefix_cache_256_32/model.bin}
        STEP_ESTIMATOR_FEATURES_PATH=${STEP_ESTIMATOR_FEATURES_PATH:-./analysis/denoise_step_prediction/models/lgb/prefix_cache_256_32/features.txt}
    elif [ "$CACHE_PREFIX" = false ] && [ "$CACHE_SUFFIX" = false ]; then
        STEP_ESTIMATOR_MODEL_PATH=${STEP_ESTIMATOR_MODEL_PATH:-./analysis/denoise_step_prediction/models/lgb/no_cache_256_32/model.bin}
        STEP_ESTIMATOR_FEATURES_PATH=${STEP_ESTIMATOR_FEATURES_PATH:-./analysis/denoise_step_prediction/models/lgb/no_cache_256_32/features.txt}
    fi

    export STEP_ESTIMATOR_MODEL_PATH STEP_ESTIMATOR_FEATURES_PATH

    # Default ShareGPT benchmark configuration (can be overridden in config)
    TOKENIZER=${TOKENIZER:-"GSAI-ML/LLaDA-8B-Instruct"}
    DATASET_PATH=${DATASET_PATH:-"eval/sharegpt_gpt4_clean_train.jsonl"}
    MAX_REQUEST_LEN=${MAX_REQUEST_LEN:-4096}
    LIMIT=${LIMIT:-}
    NUM_REQUESTS=${NUM_REQUESTS:-}
    LOG_LATENCIES=${LOG_LATENCIES:-0}
    FAIL_ON_ERROR=${FAIL_ON_ERROR:-0}
    VERBOSE=${VERBOSE:-1}
    TRUST_REMOTE_CODE=${TRUST_REMOTE_CODE:-0}

    local vllm_cmd
    vllm_cmd="RAY_DEDUP_LOGS=0 CUDA_LAUNCH_BLOCKING=1 vllm serve --trust-remote-code ${MODEL} \
    ...
    local sharegpt_cmd
    sharegpt_cmd="python eval/run_sharegpt_with_drop.py \
    --tokenizer \"$TOKENIZER\" \
    --max_request_len $MAX_REQUEST_LEN \
    --dataset_path \"$DATASET_PATH\" \
    --ip_ports localhost:${VLLM_PORT} \
    --log_filename \"$log_dir/benchmark.log\""

    if [ -n "${ARRIVAL_TIME_FILE:-}" ]; then
        sharegpt_cmd+=" --arrival_time_file \"$ARRIVAL_TIME_FILE\""
    fi
    if [ -n "${LIMIT:-}" ]; then
    ...
    echo "# Commands used for this experiment" > "$run_dir/commands.txt"
    echo "$vllm_cmd" >> "$run_dir/commands.txt"
    echo "$sharegpt_cmd" >> "$run_dir/commands.txt"
    ...
    run_logged "$sharegpt_cmd" "$log_dir/sharegpt_run.log"
    run_logged "python analysis/sharegpt_slo_attainment/run.py --path \"$log_dir/benchmark_latency_info.json\" --slo $SLO" "$log_dir/slo_attainment.log"
...
metric_patterns = {
    "slo_attainment": r"SLO Attainment \(@ .*? sec\): ([0-9.]+)%",
    "avg_latency": r"Average latency: ([0-9.]+)",
    "dropped_percentage": r"Dropped requests: ([0-9.]+)%",
}

rows = []
for name, run_dir in zip(experiment_names, experiment_run_dirs):
    log_path = run_dir / "logs" / "slo_attainment.log"
...
with summary_path.open("w") as f:
    ...
    f.write(f"{'experiment':40} | {'slo':>8} | {'avg_latency':>12} | {'drop(%)':>10}\n")
    f.write("-" * 78 + "\n")

    numeric_keys = ["slo_attainment", "avg_latency", "dropped_percentage"]
    aggregate = {key: [] for key in numeric_keys}

    for name, metrics in rows:
        if "error" in metrics:
            f.write(f"{name:40} | {'ERROR':>8} | {metrics['error']}\n")
            continue
        ...
        f.write(
            f"{name:40} | {fmt_float(metrics.get('slo_attainment', 'N/A'), 8, 4)} | "
            f"{fmt_float(metrics.get('avg_latency', 'N/A'), 12, 4)} | "
            f"{fmt_float(metrics.get('dropped_percentage', 'N/A'), 10, 4)}\n"
        )
